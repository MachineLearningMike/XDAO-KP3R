...
library IntegralMath {
    ...
    // does not revert on overflow
    function unsafeMul(uint256 x, uint256 y) internal pure returns (uint256) {
        return x * y;
    }

    // does not overflow
    function mulModMax(uint256 x, uint256 y) internal pure returns (uint256) {
        return mulmod(x, y, MAX_VAL);
    }

    // does not overflow
    function mulMod(uint256 x, uint256 y, uint256 z) internal pure returns (uint256) {
        return mulmod(x, y, z);
    }
